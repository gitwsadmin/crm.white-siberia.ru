;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		ids: {
			...
			pageTotalBlock: 'page-basket-total-block',
		},
		...
		init: function(parameters)
		{
			...
			this.AddGroupHeaderActions();
			this.setActivDleteButton(Object.getOwnPropertyNames(this.items).length);

			this.isInitialized = true;

			this.initMultibasket();

			BX.addCustomEvent(
				window,
				'sotbitMultibasketSwitch',
				function() {

					for (let item of this.selectedItem) {
						const checkBox = this.getCacheNode(this.ids.basketRoot).querySelector(`#basket__checkbox-${item.ID}`);
						this.addingSelectedItem(checkBox);
					}

					this.deleteBasketItems(Object.keys(this.items), false, true);
					this.items = {};
					this.sendRequest('recalculateAjax', {fullRecalculation: 'Y'});
					this.setColorFromMultibasket();
				}.bind(this),
			);

			BX.addCustomEvent(window, 'OnBasketChangeAfterMove', () => {
				this.sendRequest('recalculateAjax', {fullRecalculation: 'Y'});
			});
		},
		 ...
		 */
		setActivDleteButton: function(itemsCount, selectedCount=0) {

			const activeClass = 'basket__toolbar-btn__active';
			const multibasketActivClass = 'multibasket__title-active';
			const disabledClass = 'basket__checkbox__disabled';
			const checkedClass = 'state-checked';
			const deleteBtn = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'basket-groupe-item-delete');
			const checkBox = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'basket-gruope-item-checkbox');

			const multibaksetList = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'multibasket__title');

			if (itemsCount > 0) {
				checkBox.classList.remove(disabledClass);

			} else {
				checkBox.classList.add(disabledClass);
				checkBox.querySelector('.basket__checkbox_content').classList.remove(checkedClass);
			}

			const basket = document.getElementById('basket-item-list');

			if (selectedCount > 0) {
				deleteBtn.classList.add(activeClass);

				if (multibaksetList && this.multibasket.instance.baskets.length > 1) {
					multibaksetList.classList.add(multibasketActivClass);
				}
			} else {
				deleteBtn.classList.remove(activeClass);

				if (multibaksetList) {
					multibaksetList.classList.remove(multibasketActivClass);
					const multibaksetListWrapper = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'multibasket__otherbasket_wraper');
					multibaksetListWrapper.style = 'display: none';
				}
			}
		},

		showEmptyBasket: function(isEmpty) {
			if (isEmpty) {
				const basket_root = this.getCacheNode(this.ids.basketRoot);
				basket_root.querySelector('.index_cart-empty').style = 'display: block;';
				return;
			}

			const basket_root = this.getCacheNode(this.ids.basketRoot);
			basket_root.querySelector('.index_cart-empty').style = 'display: none;';
			const itemList = this.getCacheNode(this.ids.itemListTable);
			const header = basket_root.querySelector('.basket__header');
			if (itemList.scrollHeight === itemList.clientHeight) {
				header.style = 'padding-right: 15px;';
			} else {
				header.style = 'padding-right: 22px;';
			}
		},

		AddGroupHeaderActions: function()
		{
			const basket = document.getElementById('basket-item-list');

			const deletBtn = this.getEntity(basket, 'basket-groupe-item-delete');
			BX.bind(deletBtn, 'click', BX.delegate(function() {
				if (this.selectedItem.length > 0) {
					this.selectedItem.forEach((function(i) {
			...
			const allChecked = this.getEntity(basket, 'basket-gruope-item-checkbox');
			BX.bind(allChecked, 'click', BX.delegate(function(e) {

				const items = Array.prototype.slice.call(this.getEntities(basket, 'basket-item-checkbox'));

				const selectedId = this.selectedItem.map(function(i) {return i.ID});
				const notSelectedItems = items
					.filter(function(i) {return !selectedId.includes(i.id.replace('basket__checkbox-', ''))});

				this.gruopeItemsCheckbox(e.currentTarget, notSelectedItems.length === 0);

				if (notSelectedItems.length === 0) {
					items.forEach(function(i) {
						this.addingSelectedItem(i)

					}.bind(this));
				} else {
					notSelectedItems.forEach(function(i) {
						this.addingSelectedItem(i)
					}.bind(this));
				}
		...
		gruopeItemsCheckbox: function(element, condition)
		{
			const checked = 'state-checked';
			const checked_item = element.querySelector('span');
			if (condition) {
				checked_item.classList.remove(checked);
			} else {
				checked_item.classList.add(checked);
			}
		},

		searchEvent: function(event) {
			const itemParent = this.getCacheNode(this.ids.itemListTable)
			Array.prototype.slice.call(itemParent.childNodes)
				.forEach(function(i) {
					if (i && i.dataset && i.dataset.entity === 'basket-item') {

						const isSelected = this.selectedItem.filter(function(j) {
							return j.ID === i.getAttribute('data-id');
						});

						if (isSelected.length > 0) {
							this.addingSelectedItem(this.getEntity(i, 'basket-item-checkbox'));
						}
						i.remove();
					}
				}.bind(this));

			const basketItemTemplate = this.getTemplate('basket-item-template');

			const patrn = new RegExp(event.target.value, 'i');

			Object.values(this.items)
				.filter(function(i) {return patrn.test(i.NAME)})
				.forEach((function(i)  {
					const basketItemHtml = this.renderBasketItem(basketItemTemplate, i);
					itemParent.insertAdjacentHTML('afterBegin', basketItemHtml);
					this.bindBasketItemEvents(i);
				}).bind(this));

			const elmNum = Object.getOwnPropertyNames(this.items)
				.filter(function(i) {
					return patrn.test(this.items[i].NAME)
				}.bind(this)).length
			if (elmNum === 0) {
				this.getEntity(this.getCacheNode(this.ids.basketRoot), 'use-filter-and-empty-basket').style = 'display: block;';
			} else {
				this.getEntity(this.getCacheNode(this.ids.basketRoot), 'use-filter-and-empty-basket').style = 'display: none;';
			}
		},

		bindSearchEvent: function()
		{
			const headeParent = this.getCacheNode(this.ids.itemListWrapper);
			const input = this.getEntity(headeParent, 'search-input');
			...
			input.addEventListener('input', debounce(this.searchEvent.bind(this)))
		...
		editTotal: function(summ)
		{
			this.fillTotalBlocks(summ);
			this.showItemsCount();
		...
		fillTotalBlocks: function(summ)
		{
			const totalNode = document.getElementById(this.ids.pageTotalBlock)

			if (totalNode) {
				if (summ) {
					totalNode.innerHTML = summ;
					return
				}
				totalNode.innerHTML = this.result.TOTAL_RENDER_DATA.PRICE_FORMATED;
			}
		},
		...
		bindTotalEvents: function(node)
		{
			...
			const parrent_node = document.getElementById('basket-page__footer');

			if (!this.result.TOTAL_RENDER_DATA.DISABLE_CHECKOUT)
			{
				const checkOutElement = document.getElementById('page-basket-checkout-button');
				BX.bind(checkOutElement, 'click', BX.proxy(this.checkOutAction, this));
			}

			BX.bind(this.getEntity(parrent_node, 'basket-coupon-input'), 'change', BX.proxy(this.addCouponAction, this));
			BX.bind(this.getEntity(parrent_node, 'basket-coupon-input'), 'paste', BX.proxy(this.pasteCouponAction, this));
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					if (!Array.isArray(result.RESTORED_BASKET_ITEMS))
					{

						this.restoreBasketItems(result.RESTORED_BASKET_ITEMS);
						Object.getOwnPropertyNames(result.RESTORED_BASKET_ITEMS).forEach(function(i) {
							delete(this.items[i])
						}.bind(this));
					}
					...
					const sendData = result.BASKET_DATA.BASKET_ITEM_RENDER_DATA
						.map(function(i) {return {PRODUCT_ID: i.PRODUCT_ID, QUANTITY: i.QUANTITY, id: i.ID}})
					BX.onCustomEvent(this, 'updateUpselingComponent', sendData)

					this.applyBasketResult(result.BASKET_DATA);

					const itemConteiner = this.getCacheNode(this.ids.itemListTable);

					if (this.changedItems.length > 0) {
						this.editBasketItems(this.changedItems);
					}

					this.editBasketItems(this.getItemsToEdit());
					this.editTotal(result.BASKET_DATA.allSum_FORMATED);
					this.adjustBasketWrapperHeight();
					...
					const itemIds = this.getItemsAfter();
					...
					// console.log(result.BASKET_DATA.BASKET_ITEM_RENDER_DATA.length, this.selectedItem.length, result.DELETED_BASKET_ITEMS.length)
					if(result.BASKET_DATA.BASKET_ITEM_RENDER_DATA.length > this.selectedItem.length || result.DELETED_BASKET_ITEMS.length > 0 || result.BASKET_DATA.BASKET_ITEM_RENDER_DATA.length === 0) {
						const groupeCheckBox = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'basket-gruope-item-checkbox');
						this.gruopeItemsCheckbox(groupeCheckBox, true);

						if (groupeCheckBox.classList.contains('basket__checkbox__disabled') && result.BASKET_DATA.BASKET_ITEM_RENDER_DATA.length !== 0) {
							groupeCheckBox.classList.remove('basket__checkbox__disabled');
						} else if (result.BASKET_DATA.BASKET_ITEM_RENDER_DATA.length === 0) {
							this.showEmptyBasket(true);
							groupeCheckBox.classList.add('basket__checkbox__disabled');
						}

						if (result.DELETED_BASKET_ITEMS.length > 0) {
							this.setActivDleteButton(result.BASKET_DATA.BASKET_ITEM_RENDER_DATA.length)
						}
					}

					const itemBasketDeleted = document.getElementsByClassName('basket__item');
					if (itemBasketDeleted.length > 0) {
						this.showEmptyBasket(false);
					}
		...
		bindQuantityEvents: function(node, data)
		{
			...
			if (block)
			{
				...
				BX.bind(entity, 'input', BX.proxy(this.quantityChange, this));
			}
		},
		...
		bindActionEvents: function(node, data)
		{
			...
			entity = this.getEntity(node, 'basket-item-checkbox');
			BX.bind(entity, 'click', BX.proxy(this.addingSelectedItemEvent, this));
		...
		addingSelectedItemEvent: function(e)
		{
			this.addingSelectedItem(e.currentTarget);
		},

		addingSelectedItem: function(element)
		{
			const checked = 'state-checked';
			const checked_item = element.querySelector('span');
			const elementId = element.id.replace('basket__checkbox-', '');
			const itemData = this.items[elementId];
			checked_item.classList.toggle(checked);
			const isChecked = checked_item.classList.contains(checked);

			if (itemData) {
				if (isChecked) {
					this.selectedItem.push(itemData)
				} else {
					this.selectedItem = this.selectedItem.filter(function(i)  {return i.ID !== itemData.ID})
				}
			}

			const basket = document.getElementById('basket-item-list');

			const activeItemID = Array.prototype.slice.call(this.getEntities(basket, 'basket-item'))
				.map(function(i) { return i.getAttribute('data-id')});


			const allChecked = this.getEntity(basket, 'basket-gruope-item-checkbox');
			const alItems = activeItemID.filter(function(i) {
				return !this.items[i].SHOW_RESTORE
			}.bind(this))

			this.gruopeItemsCheckbox(allChecked, this.selectedItem.length !== alItems.length);

			this.setActivDleteButton(Object.keys(this.items).length , this.selectedItem.length);

			if (this.multibasket === null) {
				return;
			}

			this.multibasket.instance.setSelectedProductsInBasket(
				Object.keys(this.selectedItem).map(function(i) {
					return {
						ID: this.selectedItem[i].ID,
						PRODUCT_ID: this.selectedItem[i].PRODUCT_ID,
						NAME: this.selectedItem[i].NAME,
					};
				}.bind(this)),
			);
		},

		updateSelectdeItem: function()
		{
			const checked = 'state-checked';
			this.selectedItem.forEach(function(i) {
				const list = this.getCacheNode(this.ids.itemListTable);
				const item = list.querySelector('#basket-item-' + i['ID']);
				const checBox = this.getEntity(item, 'basket-item-checkbox').querySelector('span');
				checBox.classList.add(checked);
			}.bind(this));
		},
		...
		{
			if (parseFloat(data.MEASURE_RATIO) !== parseInt(data.MEASURE_RATIO)) {
				data.SHOW_MEASURE_RATIO = true;
			}
		...
		setColorFromMultibasket: function()
		{
			const colorConteiner = document.querySelector('.bakset__multibakset-color');
			const headerColor = document.querySelector('.multibakset-color');

			if (
				colorConteiner instanceof HTMLDivElement
				|| headerColor instanceof HTMLDivElement
			) {
				const currentBasket = this.multibasket.instance.baskets.filter(i => i.CURRENT_BASKET)[0];
				colorConteiner.style = `background-color: #${currentBasket.COLOR}`;
				headerColor.style = `background-color: #${currentBasket.COLOR};`;

				if (currentBasket.NAME) {
					const basketTitle = headerColor.parentNode;
					const headerColorNew = headerColor.cloneNode(true);
					basketTitle.innerText = currentBasket.NAME;
					basketTitle.append(headerColorNew);
				}
			}
		},

		initMultibasket: function()
		{
			this.multibasket = BX.Sotbit && BX.Sotbit.MultibasketComponent
				? BX.Sotbit.MultibasketComponent
				: null;
			...
			const multibaksetTitle = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'multibasket__title');
			const multibaksetListWrapper = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'multibasket__otherbasket_wraper');

			multibaksetTitle.addEventListener('click', function() {

				const activeClass = 'multibasket__title-active';
				if (!multibaksetTitle.classList.contains(activeClass)) {
					return;
				}

				if (multibaksetListWrapper.getAttribute('style') === 'display: none;') {
					multibaksetListWrapper.style = 'display: block;';
				} else {
					multibaksetListWrapper.style = 'display: none;';
				}
