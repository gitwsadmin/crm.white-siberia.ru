...
BX.saleAccountPay = (function()
{
	var classDescription = function(params)
	{
		...
		this.wrapper = document.querySelector('.widget-private_invoices_content');

		this.changeInputContainer = this.wrapper.getElementsByClassName('sale-acountpay-fixedpay-container')[0];
		this.paySystemsContainer = this.wrapper.getElementsByClassName('widget-private_invoices_icons')[0];
		this.inputElement = this.wrapper.getElementsByClassName('sale-acountpay-input')[0];
		this.submitButton = this.wrapper.getElementsByClassName('send_invoices')[0];
				...
				BX.ajax(
					{
						...
						{
							BX.onCustomEvent(window, 'createPersonalPymentFromVidget', []);
